...
- discover installed FCM receiver apps without requesting `QUERY_ALL_PACKAGES`;
- maintain an app-selected Aurogon FCM allowlist and MIUI Autostart policy;
...
The app list is scoped to packages exposing a receiver for `com.google.android.c2dm.intent.RECEIVE`. Its master switch always reconciles the complete app-managed Aurogon rule. Enabling an app also enables both Xiaomi Autostart AppOps and applies its currently selected battery policy. Disabling the master switch removes the app from this app's Aurogon rule and applies Optimized battery behavior. Packages in `HYPEROS_AUTO_UNRESTRICTED_PACKAGES` (currently only `com.tencent.mm`) default to master on, Autostart on, periodic enforcement on, and Don’t change; other packages default to master off, Autostart off, periodic enforcement off, and Unrestricted. That package set is used only to seed defaults. Individual controls use targeted Shizuku operations, so changing one setting does not run the full enforcement pass. Only the explicit **Apply now** action processes every saved app; startup uses the same per-app filter as periodic enforcement when the last pass is stale.
