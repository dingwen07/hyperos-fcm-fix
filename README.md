...
The app list is scoped to non-system packages exposing a receiver for `com.google.android.c2dm.intent.RECEIVE`; system and updated-system apps are excluded, while previously saved packages remain visible if they are no longer installed. Enabling an app activates its saved detail configuration. On a first enable, Aurogon and both Xiaomi Autostart AppOps are enabled; later enable/disable cycles remember the Aurogon, Autostart, battery, and periodic-enforcement selections. Disabling an app removes it from this app's Aurogon rule and applies Optimized battery behavior once without changing the saved detail configuration. All later startup, periodic, and manual full passes exclude that app until it is enabled again. Aurogon is an independent detail switch: disabling it does not disable the app or stop its other configured policies. Packages in `HYPEROS_AUTO_UNRESTRICTED_PACKAGES` (currently `com.tencent.mm` and `org.telegram.messenger`) default to app enabled, Aurogon on, Autostart on, periodic enforcement on, and Optimized; other packages default to app disabled, Autostart off, periodic enforcement off, and Don’t change. That package set is used only to seed defaults. Individual controls use targeted Shizuku operations, so changing one setting does not run the full enforcement pass. The explicit **Apply now** action processes every enabled app; startup uses the same enabled-app guard plus the per-app periodic-enforcement filter when the last pass is stale.

Don’t change is always shown for AOSP Battery optimization. When enabled, it disables the Unrestricted/Optimized/Restricted selector and suppresses all AOSP battery commands while retaining the selected value. Turning Don’t change off re-enables the selector and applies that retained value. Ordinary apps retain Optimized as their initial selection with Don’t change enabled; the packages in `HYPEROS_AUTO_UNRESTRICTED_PACKAGES` start with Don’t change disabled and Optimized active.
