...
- periodically keep WeChat (`com.tencent.mm`) at Optimized or Restricted instead of Unrestricted;
...
Android's device-idle allowlist is global per application ID. Removing WeChat from that allowlist is therefore not user-scoped; all other user-aware changes target only users 0 and 999.
